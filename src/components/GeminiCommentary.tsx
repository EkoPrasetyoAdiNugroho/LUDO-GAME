...
export const GeminiCommentary: React.FC<GeminiCommentaryProps> = ({
  ...
  return (
    <div className="bg-white border-2 sm:border-4 border-slate-900 rounded-2xl sm:rounded-3xl flex flex-col h-[380px] sm:h-[450px] lg:h-[520px] shadow-[4px_4px_0px_rgba(14,165,233,1)] sm:shadow-[6px_6px_0px_rgba(14,165,233,1)] overflow-hidden">
      {/* 1. BUNG LUDO AI COMMENTATOR HEADER (Prominent Bubble) */}
      <div className="bg-amber-50/50 border-b-2 sm:border-b-4 border-slate-900 p-2.5 sm:p-4 shrink-0 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-2.5">
          <div className="relative">
            <span className="text-xl sm:text-2xl" role="img" aria-label="Commentator Microphone">🎙️</span>
            <div className="absolute -bottom-1 -right-1 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-brand-orange border-2 border-slate-900 rounded-full animate-ping" />
          </div>
          <div className="flex flex-col">
            <span className="text-[9px] sm:text-[10px] font-sans font-black text-brand-orange uppercase tracking-widest flex items-center gap-1">
              <Sparkles size={10} className="fill-brand-orange animate-pulse text-brand-orange" /> LIVE COMMENTATOR
            </span>
            <span className="text-xs sm:text-sm font-black text-slate-900 uppercase tracking-wider font-sans">Bung Ludo</span>
          </div>
        </div>
        ...
        <button
          ...
          className={`px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl transition-all border-2 font-sans text-[9px] sm:text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1 sm:gap-1.5 cursor-pointer ${
            isMuted
        ...
        >
          <Volume2 size={13} className={isMuted ? 'opacity-40' : 'animate-pulse'} />
      ...
      {latestCommentary && (
        <div className="bg-white border-b-2 border-slate-100 p-2 sm:p-3 shrink-0">
          <motion.div
            key={latestCommentary.id}
            initial={{ opacity: 0, y: -5 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-amber-50 border-2 border-brand-orange p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl flex items-start gap-2 sm:gap-2.5 shadow-sm"
          >
            <span className="text-lg sm:text-xl shrink-0">🎙️</span>
            <p className="text-[11px] sm:text-xs font-sans font-black text-brand-orange italic leading-relaxed">
